...
use crate::some_enum::SomeEnum;
use crate::some_enum::SomeEnumWrapper;
...
fn wire_ensure_generation_impl(port_: MessagePort) {
    FLUTTER_RUST_BRIDGE_HANDLER.wrap(
        WrapInfo {
            debug_name: "ensure_generation",
            port: Some(port_),
            mode: FfiCallMode::Normal,
        },
        move || move |task_callback| Ok(ensure_generation()),
    )
}
...
impl support::IntoDart for SomeEnum {
    fn into_dart(self) -> support::DartAbi {
        match self {
            Self::A => 0,
            Self::B => 1,
...
impl support::IntoDart for SomeEnumWrapper {
    fn into_dart(self) -> support::DartAbi {
        vec![self.0.into_dart()].into_dart()
    }
}
impl support::IntoDartExceptPrimitive for SomeEnumWrapper {}

impl support::IntoDart for SomeStruct {
    fn into_dart(self) -> support::DartAbi {
        vec![self.problem.into_dart()].into_dart()
    }
}
